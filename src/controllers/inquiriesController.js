...
exports.getOne = async (req, res) => {
  try {
    const [rows] = await db.query(
      "SELECT * FROM inquiries WHERE id = ? AND admin_id = ? AND deleted_at IS NULL",
...
exports.remove = async (req, res) => {
  try {
    const [result] = await db.query(
      "UPDATE inquiries SET deleted_at = CURRENT_TIMESTAMP WHERE id = ? AND admin_id = ? AND deleted_at IS NULL",
      [req.params.id, req.admin.id]
    );
    if (!result.affectedRows) return res.status(404).json({ success: false, message: "Inquiry not found" });
